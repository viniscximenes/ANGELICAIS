...
- **Operacionais (gestor):** `/reports/consolidado` (+ `/analitico`), `/reports/tempo-indisponibilidade`, `/kpi/operadores`, `/kpi/gestor`, `/configuracoes/equipe`. `/reports/tempo-indisponibilidade/analitico` foi fundida na própria `/reports/tempo-indisponibilidade` e hoje só existe como redirect.
...
| `/reports/tempo-indisponibilidade` | `reports/tempo-indisponibilidade/page.tsx` (seção: `components/dashboard/tempo-indisponibilidade/tempo-indisp-section.tsx`) | Painel do Gestor — tabela única por operador (tempo logado + indisponibilidade + NR17/Particular/Monitoramento/Feedback, base BASE-2), cards de resumo, tabela de pausas detalhadas e dialog por operador com aderência (real x `base_pausas_programadas`, tolerância `configAderencia.toleranciaMin`) | **GESTOR** | `revalidate = 300`. Fusão desta rota com a antiga `/analitico` (2026) — conteúdo do analítico incorporado na própria página. Dois datasets do mesmo upload de BASE-2; se um vier vazio → tela de erro. Um único botão "Copiar tabela" (`[data-tempo-indisp-png]`) — exportação **sempre força nome fantasia**. Nome fantasia só na tabela unificada; cards, tabela de pausas e dialog do operador usam sempre **nome real**. `/reports/tempo-indisponibilidade/analitico` virou redirect pra cá. |
...
- Tempo Logado & Indisponibilidade → `components/dashboard/tempo-indisponibilidade/*` (raiz: `tempo-indisp-section.tsx`)
