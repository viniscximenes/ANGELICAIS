...
export async function POST(req: Request): Promise<Response> {
  ...
  const semana = computeIndisponibilidade(dias ?? [], segundaFeira);
  ...
  const templateData: Record<string, string> = {
    ...
    indisp_total: semana.consolidado.indisp,
    nr17_total: semana.consolidado.nr17,
    part_total: semana.consolidado.part,
    outras_total: semana.consolidado.outras,
  };

  for (let i = 0; i < 6; i++) {
    const slug = SLUGS_DIAS[i];
    templateData[`dia_${slug}`] = semana.diasFormatados[i] ?? "";
    templateData[`indisp_${slug}`] = semana.dias[i]?.indisp ?? "—";
    templateData[`nr17_${slug}`] = semana.dias[i]?.nr17 ?? "—";
    templateData[`part_${slug}`] = semana.dias[i]?.part ?? "—";
    templateData[`outras_${slug}`] = semana.dias[i]?.outras ?? "—";
  }
  ...
  const templatePath = path.join(
    ...
    "public",
    "templates",
    "feedback_indisponibilidade_template.docx",
  );
  ...
  try {
    ...
    console.error("[feedback/indisponibilidade] template não encontrado:", templatePath);
  ...
  try {
    ...
    console.error("[feedback/indisponibilidade] erro docxtemplater:", err);
    return new Response("Erro na geração do documento", { status: 500 });
  }

  const nomeArquivo = `Feedback_Indisponibilidade_${operador.trim().replace(/\s+/g, "_")}_${semana.periodo.replace(/\//g, "-")}.docx`;
