...
/**
 * Primeiro dia do mês passado no formato "YYYY-MM-01", em timezone
 * Brasília. Trata virada de ano (janeiro → dezembro do ano anterior).
 */
...
/**
 * Snapshot neutro do mês passado para um operador: só valor + definição,
 * sem cálculo de status/cor. Inclui flag `hasAnyDataInBank` para a UI
 * distinguir "operador sem dados" de "ninguém tem dados ainda".
 */
export async function getPreviousMonthSnapshot(
  ...
  if (anyError) {
    console.error("[get-previous-month-snapshot] erro check:", anyError);
    return {
      hasData: false,
  ...
  if (error) {
    console.error("[get-previous-month-snapshot] erro fetch:", error);
    return {
      hasData: false,
