...
/**
 * AUX vê apenas monitorias onde é responsável. A RLS já filtra
 * automaticamente; o eq() é redundante mas explícito.
 */
export async function getMonitoriasForAux(
  auxEmail: string,
): Promise<MonitoriaWithNames[]> {
  const supabase = await createClient();
  const normalizedEmail = auxEmail.trim().toLowerCase();

  const { data, error } = await supabase
    .from("monitorias")
    .select("*")
    .eq("aux_responsible_email", normalizedEmail)
    .order("data_atendimento", { ascending: false })
    .order("created_at", { ascending: false });

  if (error) {
    console.error("[get-monitorias-for-aux] erro:", error);
    return [];
  }
  ...
  const emails = Array.from(
    new Set(monitorias.map((m) => m.operatorEmail.toLowerCase())),
  );
