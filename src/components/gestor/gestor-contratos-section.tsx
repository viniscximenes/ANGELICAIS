...
import { ContratoItem } from "@/components/d-1/contrato-item";
import { Input } from "@/components/ui/input";
import { deriveNomeOperador } from "@/lib/gestor/derive-nome-operador";
import type { GestorContrato } from "@/lib/google/gestor";

const EASE_OUT_EXPO = [0.16, 1, 0.3, 1] as const;

interface GestorContratosSectionProps {
  contratosRetidos: GestorContrato[];
  contratosCancelados: GestorContrato[];
}

/**
 * Bloco de contratos da equipe do gestor. Reusa o ContratoItem do D-1 e mostra,
 * em cada item, o operador dono (vínculo preservado na Fase 1). Toggle
 * Cancelados/Retidos + busca, como o bloco de contratos atual.
 */
export function GestorContratosSection({
  contratosRetidos,
  contratosCancelados,
}: GestorContratosSectionProps) {
  const [view, setView] = useState<"cancelados" | "retidos">("cancelados");
  const [search, setSearch] = useState("");

  const list = view === "cancelados" ? contratosCancelados : contratosRetidos;

  // Deriva o nome do operador uma vez por item (também usado na busca).
  const enriquecidos = useMemo(
    () =>
      list.map((item) => ({
        ...item,
        operadorNome: deriveNomeOperador(item.operador),
      })),
    [list],
  );

  const filtered = useMemo(() => {
    if (!search.trim()) return enriquecidos;
    const q = search.toLowerCase();
    return enriquecidos.filter(
      (item) =>
        item.cliente.toLowerCase().includes(q) ||
        item.contrato.toLowerCase().includes(q) ||
        item.operadorNome.toLowerCase().includes(q),
    );
  }, [enriquecidos, search]);
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        ...
        <Input
          type="text"
          placeholder="Buscar por cliente, contrato ou operador..."
          value={search}
