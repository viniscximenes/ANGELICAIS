...
import { deletePlanoAction } from "@/lib/config/planos/actions/delete-plano-action";
...
export function DeletePlanoButton({ plano }: Props) {
  ...
  function handleClick() {
    if (
      !confirm(
        `Apagar o plano ${plano.nome}?\n\nEsta ação não pode ser desfeita.`,
      )
    )
      return;

    startTransition(async () => {
      const r = await deletePlanoAction(plano.id);
      if (r.success) {
