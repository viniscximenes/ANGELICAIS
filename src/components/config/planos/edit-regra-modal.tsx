...
import { updateRegraAction } from "@/lib/config/planos/actions/update-regra-action";
...
export function EditRegraModal({ open, onClose, regra }: Props) {
  const router = useRouter();
  const [tempoMin, setTempoMin] = useState(String(regra.tempoMinMeses));
  const [semLimiteSuperior, setSemLimiteSuperior] = useState(
    regra.tempoMaxMeses === null,
  );
  const [tempoMax, setTempoMax] = useState(
    regra.tempoMaxMeses !== null ? String(regra.tempoMaxMeses) : "",
  );
  const [descontoMax, setDescontoMax] = useState(String(regra.descontoMaxPct));
  const [duracao, setDuracao] = useState(String(regra.duracaoMeses));
  const [isPending, startTransition] = useTransition();

  useEffect(() => {
    setTempoMin(String(regra.tempoMinMeses));
    setSemLimiteSuperior(regra.tempoMaxMeses === null);
    setTempoMax(
      regra.tempoMaxMeses !== null ? String(regra.tempoMaxMeses) : "",
    );
    setDescontoMax(String(regra.descontoMaxPct));
    setDuracao(String(regra.duracaoMeses));
  }, [regra]);
  ...
  function handleSubmit() {
    ...
    startTransition(async () => {
      const r = await updateRegraAction({
        id: regra.id,
        tempoMinMeses: min,
        tempoMaxMeses: max,
        descontoMaxPct: desc,
